...
class RepaymentTrackerLoanTest < Test::Unit::TestCase
  def setup
    ...
    @loan = Loan.new(@simulator, @lender, @borrower, @amount, Duration.in_years(25), rate, RepaymentStrategy::Standard)
  ...
  def test_after_last_monthly_payment_and_repayment_of_principal
    ...
    assert_equal -@amount + (@expected_low_monthly_payment * 150) + (@expected_high_monthly_payment * 150), @lender.cash_position
    assert_equal @amount - (@expected_low_monthly_payment * 150) - (@expected_high_monthly_payment * 150), @borrower.cash_position
  end
